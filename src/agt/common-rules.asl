...
select_bid([bid(BidAgent,Bid,ShopId,item(ItemId,Qty),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),BidWinner) :- Bid \== -1 & Bid < AuxBid & ((initiator::awarded(BidAgent,_,_,_,TaskCount) & TaskCount < 3) | (initiator::awarded_assemble(BidAgent,_,_,_,_,AssTaskCount) & AssTaskCount < 3)) & item(ItemId,Volume,_,_) & actions.getLoad(BidAgent,Load) & Load >= Volume*Qty & select_bid(Bids,bid(BidAgent,Bid,ShopId),BidWinner).
...
select_bid_tool([bid(BidAgent,Bid,ShopId,tool(ItemId),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),BidWinner) :- Bid \== -1 & Bid < AuxBid & ((initiator::awarded(BidAgent,_,_,_,TaskCount) & TaskCount < 3) | (initiator::awarded_assemble(BidAgent,_,_,_,_,AssTaskCount) & AssTaskCount < 3)) & item(ItemId,Volume,_,_) & actions.getLoad(BidAgent,Load) & Load >= Volume & select_bid(Bids,bid(BidAgent,Bid,ShopId),BidWinner).
