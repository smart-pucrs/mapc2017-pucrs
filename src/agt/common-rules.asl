...
select_bid([bid(BidAgent,Bid,ShopId,item(ItemId,Qty),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),bid(BidAgentWinner,BidWinner,ShopIdWinner)) :- Bid == -2 & not initiator::awarded(BidAgent,_,_,_,_) & BidWinner = Bid & BidAgentWinner = BidAgent & ShopIdWinner = ShopId.
select_bid([bid(BidAgent,Bid,ShopId,item(ItemId,Qty),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),BidWinner) :- Bid \== -1 & Bid \== -2 & initiator::awarded(BidAgent,ShopId,_,_,TaskCount) & TaskCount < 2 & item(ItemId,Volume,_,_) & actions.getLoad(BidAgent,Load) & Load >= Volume*Qty & select_bid([],bid(BidAgent,Bid,ShopId),BidWinner).
select_bid([bid(BidAgent,Bid,ShopId,item(ItemId,Qty),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),BidWinner) :- Bid \== -1 & Bid \== -2 & Bid < AuxBid & ((initiator::awarded(BidAgent,_,_,_,TaskCount) & TaskCount < 2) | (initiator::awarded_assemble(BidAgent,_,_,_,_,AssTaskCount) & AssTaskCount < 3)) & item(ItemId,Volume,_,_) & actions.getLoad(BidAgent,Load) & Load >= Volume*Qty & select_bid(Bids,bid(BidAgent,Bid,ShopId),BidWinner).
select_bid([bid(BidAgent,Bid,ShopId,item(ItemId,Qty),TaskId)|Bids],bid(AuxBidAgent,AuxBid,AuxShopId),BidWinner) :- Bid \== -1 & Bid \== -2 & not initiator::awarded_assemble(BidAgent,_,_,_,_,_) & not initiator::awarded(BidAgent,_,_,_,_) & Bid < AuxBid & select_bid(Bids,bid(BidAgent,Bid,ShopId),BidWinner).
